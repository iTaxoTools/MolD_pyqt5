...
class Main(QDialog, FORM_CLASS):
    def __init__(self,parent=None):
        ...
        self.setupUi(self)
        self.setWindowIcon(QIcon(resource_path('icon/MolD3.ico')))
    ...
    def Handel_Buttons(self):
        ...
        self.cb1.activated.connect(self.selectionchange)

    def selectionchange(self):
        msg= "1 for species, 2 for above species"
        QMessageBox.information(self, 'please select', msg)

    def fail(self, exception):
        print(str(exception))
        QMessageBox.warning(self, "Warning", f"The output not obtained, please check input and error is {exception}")


    def done(self, result):
        QMessageBox.information(self, "Information", "The output data generated successfully\nPlease click clear to start a new analysis")
    ...
    def download2(self):
        self.launcher.started.connect(self.started)
        self.launcher.finished.connect(self.finished)
        self.launcher.done.connect(self.done)
        self.launcher.fail.connect(self.fail)
        self.launcher.start()


    def open_file(self):
        msg = 'The input file is in fasta format: each entry contains an identifier line, which starts with ‘>’ and contains two pipe (‘|’) -separated parts:\na free-style sequence identifier\ntaxon identifier of the query level.'
    ...
    def save_file(self):
        try:
            ...
            Path(self.outpath['output']).rename(path1)
